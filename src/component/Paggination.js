    ...
    return (
        <div className="pagination-festum">
            ...
            <ul className="">
                ...
                {(allEvents.page > 2) && (allEvents.page <= allEvents.last_page) && (allEvents.page > 1 && allEvents.page < allEvents.last_page-1) &&<li onClick={() => setPageNo(allEvents?.page -2)}>
                    <button className={"btns"}><span>{allEvents?.page -2}</span></button>
                </li>}

                {(allEvents.page > 2) && (allEvents.page <= allEvents.last_page) && <li onClick={() => setPageNo(allEvents?.page -1)}>
                ...
                {(allEvents.page > 0) && (allEvents.page < allEvents.last_page) && <li onClick={() => setPageNo(allEvents?.page +1)}>
                    <button className={"btns"}><span>{allEvents?.page +1}</span></button>
                </li>}

                {(allEvents.page > 0) && (allEvents.page < allEvents.last_page -1) && <li onClick={() => setPageNo(allEvents?.page +2)}>
                    <button className={"btns"}><span>{allEvents?.page +2}</span></button>
                </li>}

                {/* {
                    Array(allEvents?.last_page).fill().map( (e,i) => (
                        <li onClick={() => setPageNo(i+1)} >
                            <button className={allEvents?.page == i+1 ? "btns active" : "btns" }><span>{i+1}</span></button>
                        </li>
                    ))
                } */}
